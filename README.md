...
// Обычный расчёт
double cost = calc.calculate(15.0, "large", false, "high");
// → 560.0 ₽  (200 + 200) × 1.4

// Хрупкий груз
double fragile = calc.calculate(10.0, "small", true, "normal");
// → 500.0 ₽  (100 + 100 + 300) × 1.0

// Минимальная стоимость
double min = calc.calculate(1.0, "small", false, "normal");
// → 400.0 ₽  (50 + 100 = 150 → минимум 400)

// Исключение: хрупкий груз дальше 30 км
calc.calculate(31.0, "small", true, "normal");
// → IllegalArgumentException

// Исключение: null в параметрах
calc.calculate(10.0, null, false, "normal");   // → IllegalArgumentException
calc.calculate(10.0, "small", false, null);    // → IllegalArgumentException
```
...
Проект покрыт **64 тестами** с использованием JUnit 5.
...
| `SizeCost`           | `basic`  | 6      | Надбавки за `small`/`large` + регистронезависимость |
| `FragilityCost`      | `basic`  | 2      | Надбавка за хрупкость                               |
| `LoadMultiplier`     | `basic`  | 8      | Все 4 коэффициента + регистронезависимость          |
...
| `ErrorHandling`      | `error`  | 12     | null/пустые/неизвестные значения, хрупкий > 30 км   |
